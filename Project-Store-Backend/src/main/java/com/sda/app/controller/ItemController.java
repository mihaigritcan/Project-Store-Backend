...
public class ItemController {
    ...
    @GetMapping("/category")
    public ResponseEntity<ApiResponse> getItemsByCategory(@RequestParam() String category) {
        List<Item> itemsList = itemService.findAllByCategory(Category.valueOf(category.toUpperCase()));
        ApiResponse response = new ApiResponse.Builder()
                .status(200)
                .message("Lista items generata cu success")
                .data(itemsList)
